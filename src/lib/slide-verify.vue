...
export default {
  ...
  props: {
    ...
    fnImgSrc: {
      type: Function,
      default: (req) => {
        console.log('fnImgSrc x ' + req.x + ' y ' + req.y)
    ...
    fnVerify: {
      type: Function,
      default: (req) => {
        console.log('fnVerify x ' + req.x + ' y ' + req.y)
        return req
      },
    },
  },
  ...
  methods: {
    ...
    getRandomImg () {
      this.block_x = this.getRandomNumberByRange(this.L + 10, this.w - (this.L + 10))
      this.block_y = this.getRandomNumberByRange(10 + this.r * 2, this.h - (this.L + 10))
      console.log('getRandomImg block_x ', this.block_x)
      return this.fnImgSrc({ x: this.block_x, y: this.block_y })
    ...
    verify () {
      ...
      let verifyRs = this.fnVerify(verifyReq)
      return verifyRs
      // const spliced = Math.abs(left - this.block_x) <= accuracy
      // console.log('verify left ' + left + ' block_x ' + this.block_x)
      // return {
      //   spliced: verifyRs && Math.abs(left - this.block_x) <= accuracy,
      //   TuringTest: average !== stddev, // equal => not person operate
      // }
    },
    reset () {
